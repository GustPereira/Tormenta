...
function item(over: Partial<InventoryItem>): InventoryItem {
  return {
    ...
    equipped: false,
    activeEffect: false,
    modifiers: { attributes: {}, skills: {}, hitPoints: 0, mana: 0, defense: 0 },
    notes: '',
    ...over,
  }
}
...
describe('Effect / ItemEffect', () => {
  it('Effect avulso é editável e ativável', () => {
    const e = new Effect({ id: 'e', name: 'Bênção', active: true, modifiers: { attributes: {}, skills: {}, hitPoints: 0, mana: 0, defense: 0 } })
...
describe('collectEffects + aggregateActiveModifiers', () => {
  it('agrega itens ativos e efeitos avulsos ativos, ignora inativos', () => {
    const c = {
      ...
      inventory: [
        item({ id: 'a', activeEffect: true, modifiers: { attributes: { forca: 2 }, skills: { atletismo: 1 }, hitPoints: 5, mana: 0, defense: 1 } }),
        item({ id: 'b', activeEffect: false, modifiers: { attributes: { forca: 99 }, skills: {}, hitPoints: 99, mana: 0, defense: 0 } }),
      ],
      effects: [
        { id: 'e1', name: 'Bênção', active: true, modifiers: { attributes: { forca: 1 }, skills: {}, hitPoints: 0, mana: 3, defense: 0 } },
        { id: 'e2', name: 'Desligado', active: false, modifiers: { attributes: { forca: 50 }, skills: {}, hitPoints: 0, mana: 0, defense: 0 } },
      ],
    }
...
describe('describeModifiers', () => {
  it('resume os modificadores em texto', () => {
    const txt = describeModifiers({ attributes: { forca: 2 }, skills: {}, hitPoints: 5, mana: 0, defense: -1 })
    expect(txt).toContain('For +2')
    expect(txt).toContain('PV +5')
    expect(txt).toContain('Defesa -1')
  })
  it('indica quando não há modificadores', () => {
    expect(describeModifiers({ attributes: {}, skills: {}, hitPoints: 0, mana: 0, defense: 0 })).toBe('sem modificadores')
  })
})
